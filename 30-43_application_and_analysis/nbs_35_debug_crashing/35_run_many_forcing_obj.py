...
def experiment_run(n_particles, p_max_initial, p_min_initial, s_0, model_name, lst_camels_forcing,
...
def run(lst_camels_forcing, HRU_id, sigma_p_Sf, n_particles):
    ...
    ds_summary = experiment_run(n_particles, p_max_initial, p_min_initial, s_0, model_name, lst_camels_forcing,
                                                paths, HRU_id, sigma_tuple, H, assimilate_window)
...
def main():
    ...
    for n_particles in lst_n:
        for index, HRU_id_int in enumerate(["01181000"]):
            if index == 0:
                lst_sig_p = [0.3, 0.2, 0.1, 0.01]
            else:
                lst_sig_p = [0.5, 0.4, 0.3, 0.2, 0.1, 0.01]
            for sigma_p_Sf in lst_sig_p:
                ...
                for _ in range(n_particles):
                    lst_camels_forcing.append(sources.HBVForcing(start_time=experiment_start_date,
                ...
                current_time = str(datetime.now())[:-10].replace(":", "_")
                print(f'starting sig-{sigma_p_Sf} with N-{n_particles} at {current_time}')
                run(lst_camels_forcing, HRU_id, sigma_p_Sf, n_particles)


                # remove temp file once run - in case of camels just one file
                for forcing in lst_camels_forcing:
