...
HRU_ids = [path.name[1:8] for path in  forcing_path.glob("*_lump_cida_forcing_leap.txt")]
...
def run_callibration(HRU_id):
    ...
    ensmble_best_run = df_ensemble[f'particle {np.array(lst_nse).argmax()}'].copy()
    
    state_vector = lst_state_vector[-1][np.array(lst_nse).argmax()].copy()

    del df_ensemble, lst_Q_cal, ensemble, lst_state_vector, lst_nse, df_Q, df_Q_Cal
...
def main():
    ...
    for hruid in HRU_ids[7:]:
        current_time = str(datetime.now())[:-10].replace(":","_")
        print(f'{hruid} starting at {current_time}')
        ensmble_best_run, state_vector = run_callibration(hruid)
        current_time = str(datetime.now())[:-10].replace(":","_")
        np.savetxt(output_path / f"nse_{hruid}_{current_time}.txt", np.array([ensmble_best_run]))
        np.savetxt(output_path / f"state_vectors_{hruid}_{current_time}.txt", np.array(state_vector))
        del ensmble_best_run, state_vector
        print(f'{hruid} ending at {current_time}')
